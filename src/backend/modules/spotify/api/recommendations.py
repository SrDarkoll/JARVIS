    ...
    try:
        ...
        while True:
            ...
            for p in items:
                ...
                if accessible and name.lower() == SPOTIFY_AUTOMIX_PLAYLIST_NAME.lower():
                    return p
...
def _spotify_obtener_tracks_playlist(playlist_id: str, max_items: int = 300) -> list[dict]:
    ready, _ = _spotify_ready()
    if not ready:
        return []
    try:
...
def _spotify_obtener_uris_playlist(playlist_id: str, max_items: int = 300) -> list[str]:
    return [track["uri"] for track in _spotify_obtener_tracks_playlist(playlist_id, max_items=max_items)]
...
def _spotify_reemplazar_playlist_con_uris(playlist_id: str, uris: list[str]) -> bool:
    ...
    try:
        ...
        actual_uris = _spotify_obtener_uris_playlist(playlist_id, max_items=len(uris_limpias) + 5)
...
def _spotify_current_user_top_tracks(limit: int = 10) -> list[dict]:
    ...
    for time_range in ("short_term", "medium_term"):
        try:
            page = (
                client.sp.current_user_top_tracks(
    ...
    for artist_id in artist_ids[:3]:
        ...
        try:
            ...
            for genre in artist.get("genres", []) or []:
                ...
                if normalized_genre and normalized_genre not in genres:
...
def _spotify_current_user_top_artists(limit: int = 8) -> list[dict]:
    ...
    try:
        page = (
            client.sp.current_user_top_artists(
                limit=max(1, min(int(limit), 10)),
                time_range="medium_term",
            )
            or {}
        )
        return [artist for artist in page.get("items", []) or [] if isinstance(artist, dict)]
    except Exception as error:
...
def _spotify_audio_feature_queries(track_id: str | None) -> list[str]:
    if not SPOTIFY_EXTENDED_QUOTA_MODE or not track_id or client.sp is None or not hasattr(client.sp, "audio_features"):
        return []
    try:
        ...
        while True:
            ...
            for playlist in items:
                if not isinstance(playlist, dict):
                    continue
                if str(playlist.get("name") or "").strip().lower() != SPOTIFY_AUTOMIX_PLAYLIST_NAME.lower():
                    continue
...
def _spotify_dynamic_mix_candidates(track: dict, limit: int) -> list[dict]:
    ...
    genres = []
    for genre in _spotify_artist_genres(seed_artist_ids) + _spotify_user_top_genres(top_artists):
        if genre and genre not in genres:
            genres.append(genre)

    queries = [f'genre:"{genre}"' for genre in genres[:4]]
    queries.extend(f'artist:"{name}"' for name in seed_artist_names[:2])
    queries.extend(f'artist:"{artist["name"]}"' for artist in top_artists[:3] if artist.get("name"))
    if seed_name:
        artist_query = f' artist:"{seed_artist_names[0]}"' if seed_artist_names else ""
        queries.append(f'track:"{seed_name}"{artist_query}')
        queries.append(" ".join([seed_name, *seed_artist_names[:1]]).strip())
...
def _spotify_extended_quota_candidates(track: dict, limit: int) -> list[dict]:
    ...
    artist_ids = [artist.get("id") for artist in track.get("artists") or [] if artist.get("id")]
    candidates = []
    ...
    def _artist_top_tracks(artist_id: str) -> list[dict]:
        ...
        try:
            if market:
                return (client.sp.artist_top_tracks(artist_id, country=market) or {}).get("tracks", [])
...
def _spotify_obtener_similares(track: dict, limite: int = SPOTIFY_RADIO_QUEUE_SIZE):
    ...
    print(f"  [SIMILARES] Construyendo mix contextual para: '{seed_name}' | Artistas: {seed_artist_names}")
    ...
    if album_id and len(similares) < limite:
        ...
        seed_index = next(
            (i for i, item in enumerate(album_tracks) if item.get("id") == track_id or item.get("uri") == track_uri),
            -1,
        )
        if seed_index >= 0:
    ...
    artistas_result = list({a.get("name") for t in similares for a in (t.get("artists") or []) if a.get("name")})
    print(f"  [SIMILARES] Encontrados: {len(similares)} tracks | Artistas en mix: {artistas_result[:5]}")
    return similares
...
def _spotify_esperar_track(track_id: str, intentos: int = 4, delay: float = 0.35) -> bool:
...
def _spotify_iniciar_determinista(track_uri: str, track_id: str, lista_uris: list[str], device_id: str | None) -> bool:
    ...
    if _spotify_start_playlist_like(lista, device_id) or _spotify_start_playlist_like(lista, None):
        if _spotify_esperar_track(track_id, intentos=5, delay=0.30):
            return True
