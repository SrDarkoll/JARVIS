...
def _profile_scope(profile_id: str | None) -> str:
    ...
    if pid == DEFAULT_PROFILE_ID: return "owner"
    if pid == SHARED_PROFILE_ID: return "shared"
    return "guest"

def _texto_limpio_memoria(texto: str) -> str:
    out = str(texto or "").replace("\x00", " ")
    try:
        if services.reparar_unicode: out = services.reparar_unicode(out)
    except: pass
...
def _normalizar_destino_web(destino: str) -> str:
    ...
    if not d: return "https://www.google.com"
    d = re.sub(r"^(abre|habre|abreme|ir a|entrar a|busca|pon)\s+", "", d)
    if "youtube" in d and "." not in d: d = "youtube.com"
    if "facebook" in d and "." not in d: d = "facebook.com"
    if "google" in d and "." not in d: d = "google.com"
    if "github" in d and "." not in d: d = "github.com"
    if "." not in d: d = f"{d}.com"
    if not d.startswith(("http", "www")): d = f"https://{d}"
    return d
