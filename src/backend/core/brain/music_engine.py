...
def _es_posible_titulo_cancion(user_input: str) -> bool:
    ...
    if not t or "?" in t or "http" in t:
        return False
    if _es_comando_repetir_musica(t):
        return False
    if len(t) > 90:
        return False
    ...
    if any(b in t for b in bloqueantes):
        return False
    return True
...
def _contexto_musica_activo(historial: list | None = None) -> bool:
    ...
    for m in reversed(historial):
        ...
        if not txt:
            continue
        if not ultimo_user and isinstance(m, HumanMessage):
            ultimo_user = txt
        elif not ultimo_ai and isinstance(m, AIMessage):
            ultimo_ai = txt
        if ultimo_user and ultimo_ai:
            break
